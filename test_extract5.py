"""TEST v5 - final heading extractor."""
import sys
import re
...
def detect_structure(lines):
    ...
    # Find References heading page — stop after this
    stop_page = None
    for l in lines:
    ...
    for l in lines:
        ...
        if m:
            ...
            top = parts[0]
            # Detect restart (appendix list): top-level number goes backwards
            if len(parts) == 1:
                if top <= max_chapter and max_chapter > 3:
                    # Restart — likely appendix list, skip
                    continue
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python test_extract5.py <pdf>")
        sys.exit(1)
    pdf = sys.argv[1]
